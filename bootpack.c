...
struct proAndCon *proAndCon_init(struct MEMMAN *memman);
...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	/* pac */ 
	struct proAndCon *pac;  //å…±äº«å…¨å±€å˜é‡
...
struct proAndCon *proAndCon_init(struct MEMMAN *memman) {
	struct proAndCon *pac;
	pac = (struct proAndCon *) memman_alloc_4k(memman, sizeof (struct proAndCon));
	pac->in = 0;
	pac->out = 0;
	pac->size = BUF_SIZE;

	struct semaphore *full;
	full = (struct semaphore *) memman_alloc_4k(memman, sizeof (struct semaphore));
	full->value = 0;
	full->first = -1;
	full->last = -1;
	pac->full = full;
	...
	empty = (struct semaphore *) memman_alloc_4k(memman, sizeof (struct semaphore));
	empty->value = pac->size;
	empty->first = -1;
	empty->last = -1;
	pac->empty = empty;

	struct semaphore *mutex;
	mutex = (struct semaphore *) memman_alloc_4k(memman, sizeof (struct semaphore));
	mutex->value = 1; //only one process can go into the buf at one time
	mutex->first = -1;
	mutex->last = -1;
	pac->mutex = mutex;	
